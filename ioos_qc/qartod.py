...
@add_flag_metadata(
    standard_name="location_test_quality_flag", long_name="Location Test Quality Flag"
)
def location_test(
    lon: Sequence[N],
    lat: Sequence[N],
    ...
    if lon.shape != lat.shape:
        raise ValueError(
            "Lon ({0.shape}) and lat ({1.shape}) are different shapes".format(lon, lat)
        )
...
def gross_range_test(
    inp: Sequence[N], fail_span: Tuple[N, N], suspect_span: Tuple[N, N] = None
) -> np.ma.core.MaskedArray:
    ...
    if suspect_span is not None:
        ...
        if uspan.minv < sspan.minv or uspan.maxv > sspan.maxv:
            raise ValueError(
                "Suspect {} must fall within the Fail {}".format(uspan, sspan)
            )
...
class ClimatologyConfig(object):
    ...
    mem = namedtuple("window", ["tspan", "fspan", "vspan", "zspan", "period"])
    ...
    def add(
        ...
        if period is not None:
            ...
            tspan = span(*sorted([pd.Timestamp(tspan[0]), pd.Timestamp(tspan[1])]))
    ...
    def check(self, tinp, inp, zinp):
        ...
        for m in self._members:
            ...
            if not isnan(m.zspan):
                ...
                with np.errstate(invalid="ignore"):
                    z_idx = (
                        (~zinp.mask) & (zinp >= m.zspan.minv) & (zinp <= m.zspan.maxv)
                    )
            else:
...
@add_flag_metadata(
    standard_name="spike_test_quality_flag", long_name="Spike Test Quality Flag"
)
def spike_test(
    inp: Sequence[N],
    ...
    if method == "average":
        ...
        raise ValueError(
            'Unknown method: "{0}", only "average" and "differential" methods are available'.format(
                method
            )
        )
...
@add_flag_metadata(
    standard_name="flat_line_test_quality_flag", long_name="Flat Line Test Quality Flag"
)
def flat_line_test(
    inp: Sequence[N],
    tinp: Sequence[N],
...
def attenuated_signal_test(
    ...
    **kwargs,
    ...
    if check_type == "std":
        ...
        raise ValueError(
            'Check type "{}" is not one of ["std", "range"]'.format(check_type)
        )
...
def density_inversion_test(
    ...
    if inp.shape != zinp.shape:
        raise ValueError(
            f"Density ({inp.shape}) and depth ({zinp.shape}) must be the same shape"
        )
    ...
    if suspect_threshold is not None:
        with np.errstate(invalid="ignore"):
            ...
            if any(is_suspect):
                flag_arr[:-1][
                    is_suspect == True
                ] = QartodFlags.SUSPECT  # noqa:E712- Previous value
                flag_arr[1:][
                    is_suspect == True
                ] = QartodFlags.SUSPECT  # noqa:E712- Reversed value
    ...
    if fail_threshold is not None:
        with np.errstate(invalid="ignore"):
            ...
            if any(is_fail):
                flag_arr[:-1][
                    is_fail == True
                ] = QartodFlags.FAIL  # noqa:E712- Previous value
                flag_arr[1:][
                    is_fail == True
                ] = QartodFlags.FAIL  # noqa:E712- Reversed Value
